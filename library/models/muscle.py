...
class MuscleState(typing.NamedTuple):
    muscle_l_previous: float #lenght of muscle
    ...
    def step(self, params: MuscleParams, act: float, joint_angle: float):
        return muscle_step(params, self, act, joint_angle)
...
def muscle_step(params: MuscleParams, state: MuscleState, act: float, joint_angle: float):
    ...
    torque = calculate_torque(muscle_l, params.con_p1, params.con_p2, muscle_force) #takes connection point 1 as the pole that is used to calculate the torque
    return MuscleState(muscle_l, act, muscle_force, Fce, Fpe, muscle_v, torque), torque
...
def PE(pe_shape, pe_xm, norm_length_pe, muscle_maxf):
    l_pe = norm_length_pe
    heaviside_output = superspike(l_pe - 1)  # Shift the Heaviside step function to be 0 for x < 1
